...
function create_user($firstname,$lastname,$id_number,$department,$date_of_birth,$gender,$district,$branch,$cell,$province_id,$position, $position_for)
{
    ...
    global $db;
    try {
        $stmt = $db->prepare("INSERT INTO people (id,firstname,lastname,id_no,department,DOB,gender,district,branch,cell,province_id,position,position_for) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)");
        $stmt->execute(array($id,$firstname,$lastname,$id_number,$department,$date_of_birth,$gender,$district,$branch,$cell,$province_id,$position, $position_for));
